...
> **About the screenshots and data shown here.** Every name, dollar amount, address, and document in the images below comes from a synthetic mock dataset generated by `grant-management-system/mock/generate_mock.py`. Organizations, staff, programs, awards, and document contents are Faker-generated; geographic polygons are public TIGER/Line boundaries. Nothing in these screenshots represents my employer's operational data, real funders, or real grant outcomes. The mock exists so the application can be demonstrated without exposing any production information.
